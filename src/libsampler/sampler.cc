...
void SignalHandler::FillRegisterState(void* context, RegisterState* state) {
...
#else  // !__DARWIN_UNIX03
  state->pc = reinterpret_cast<void *>(mcontext->ss.pc);
  state->sp = reinterpret_cast<void *>(mcontext->ss.sp);
  state->fp = reinterpret_cast<void *>(mcontext->ss.r[7]);
#endif  // __DARWIN_UNIX03
#elif V8_HOST_ARCH_ARM64
#if __DARWIN_UNIX03
  state->pc = reinterpret_cast<void *>(mcontext->__ss.__pc);
  ...
  state->pc = reinterpret_cast<void *>(mcontext->ss.pc);
  state->sp = reinterpret_cast<void *>(mcontext->ss.sp);
  state->fp = reinterpret_cast<void *>(mcontext->ss.fp);
#endif  // __DARWIN_UNIX03
#endif  // V8_HOST_ARCH_*
#elif V8_OS_FREEBSD
#if V8_HOST_ARCH_IA32
